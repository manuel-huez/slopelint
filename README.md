...
Semantic similarity uses a shared HTTP embedding server, so the Go binary builds
on every supported OS without CGO. Set `SLOPELINT_LLAMA_URL` to the server URL;
it defaults to `http://127.0.0.1:8080`. The server owns the fixed
`jina-embeddings-v2-base-code` model, 768-dimensional output, and mean pooling.
Set up that service separately; slopelint does not download model files or start
...
The HTTP listener stays available during idle sleep; the next embedding request
reloads the model. A shared router can use the same model alias and settings.
Do not put different weights or pooling behind this alias: cached vectors and
similarity thresholds depend on this exact configuration.
...
the shared server, reuse the content-addressed vector cache, and write
...
before retrying missing IDs. Embedding requests use the shared HTTP server and
do not load native libraries in the Go process.
...
- `SLOPELINT_LLAMA_URL`: shared embedding server URL; default
  `http://127.0.0.1:8080`
...
- `os.UserCacheDir()/slopelint/analysis-v13`
...
With the embedding server running, verify the real HTTP similarity path:
