...
func repoAnalysisCacheKey(
	...
	absoluteDir := filepath.Join(
		location.sourceRoot,
		filepath.FromSlash(location.relativeDir),
	)
...
func repositoryCacheLocationForDir(dir string) (repositoryCacheLocation, error) {
	...
	resolvedDir := absoluteDir
	if resolved, resolveErr := filepath.EvalSymlinks(resolvedDir); resolveErr == nil {
		resolvedDir = resolved
	}

	sourceRoot, objectFormat, commonDir, gitErr := repoAnalysisGitInfo(resolvedDir)
	...
	if gitErr == nil {
		...
		sourceRoot, err = findGoModuleRoot(resolvedDir)
		if err != nil {
			return repositoryCacheLocation{}, err
		}
	...
	relativeDir, err := filepath.Rel(sourceRoot, resolvedDir)
	if err != nil || !filepath.IsLocal(relativeDir) {
		return repositoryCacheLocation{}, errAnalysisCacheDisabled
	}

	relativeRoot, err := filepath.Rel(resolvedDir, sourceRoot)
	if err != nil {
		return repositoryCacheLocation{}, err
	}

	sourceRoot = filepath.Clean(filepath.Join(absoluteDir, relativeRoot))
...
func repoAnalysisGitDigest(
	...
	jobs.Go(func() {
		untracked, errs[2] = repoAnalysisGitOutput(
			root,
			"untracked files",
			[]string{gitListFilesCommand, "--others", "--exclude-standard", "-z", "--"},
			pathspecs...,
		)
	})
	jobs.Go(func() {
...
func repoAnalysisGitIndex(root string, pathspecs []string) (map[string]string, error) {
	index, err := repoAnalysisGitOutput(
		root,
		"tracked files",
		[]string{gitListFilesCommand, "--stage", "-z", "--"},
		pathspecs...,
	)
	if err != nil {
		return nil, err
...
func repoAnalysisSubmodulesSupported(root string) (bool, error) {
	...
	cmd := exec.Command("git", gitListFilesCommand, "--stage", "-z")
	cmd.Dir = root
